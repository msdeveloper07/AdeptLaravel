<html>
    ...
    <body>
        <table width='100%' cellspacing="0.00001%" cellpadding="5px">
            ...
            <?php $total = array();  ?>
            @foreach($goodsin as $k=>$j)
            @if(in_array($j->goodsin_id,$array) && count($array)>0)

            <?php 
            if ($j->goodsin_type == "Weekly/Dimension") {
                $charge_rate = array();
                  
                $lots = \App\Models\Lot::where('goodsin_id', $j->goodsin_id)->get();
             
                $month = $dateM;
                $year = $dateY;
                $period = \App\Models\Period::where('year',$year)->where('period_month_id',$month)->first();
                $start_date = $period->start_date;
                $month_last   = $period->end_date;
                
              // echo $month.'-'.$year.'-'.$start_date.'-'.$month_last;die();
                $del = App\Models\Delivery::where('goodsin_id', $j->goodsin_id)->orderBy('id', 'DESC')->first();
         
                    ?>
                    @foreach($lots as $l)
                    <?php
//                      
                    $delivery_lot = App\Models\DeliveryLot::where('goodsin_id', $j->goodsin_id)->where('lot_id',$l->lot_id)->orderBy('delivery_id', 'DESC')->first();
                    $volume_delivered = App\Models\DeliveryLot::where('goodsin_id', $j->goodsin_id)->where('lot_id',$l->lot_id)->lists('volume_to_deliver')->toArray();
                   ...
                   //App\Libraries\ZnUtilities::pa($delivery);die;
                    ...
                    $last_date =  $month_last;
                    
                     $goods_in_d = explode('-', $goods_in_date);
                       $goods_in_date_goodsin = $j->goods_in_date;
                       if($goods_in_date_goodsin >= $start_date && $goods_in_date_goodsin <= $month_last){
                        $handling_charge = $j->handling_charge;
                    }else{
                        $handling_charge = '0.00';
                    }
                    if (strtotime($goods_in_date) >= strtotime($start_date)&&strtotime($goods_in_date) <= strtotime($month_last)) {
                      

//                    }else if($goods_in_date < $start_date||$goods_in_date > $month_last){
//                         $goods_in_date = $start_date; 
                    }else{
                          $goods_in_d[1] = $dateM;
                 ...
                 //  echo $till_date;
                    ...
                    if ($till_date != '') {
                        ...
                        if (strtotime($till_date) >= strtotime($start_date)&&strtotime($till_date) <= strtotime($month_last)) {
                        }else{
                            $till_d[1] = $dateM;
...
//                            \App\Libraries\ZnUtilities::pa($j->goodsin_id.'  Lot'.$l->lot_id);
...
//                            \App\Libraries\ZnUtilities::pa('total  '.$count*$l->volume*$j->charge_rate);
                           if($till_date){  
                    $delivery_old = App\Models\Delivery::where('goodsin_id', $j->goodsin_id)->where('delivery_date','<=',$till_date)->orderBy('job_delivery_id','ASC')->get();    
...
//                         echo $dateM;
                    ...
                    $volume_delivered1[] = App\Models\DeliveryLot::where('goodsin_id', $j->goodsin_id)->where('lot_id',$l->lot_id)->where('delivery_id',$do->id)->pluck('volume_to_deliver');           
//                     App\Libraries\ZnUtilities::pa($volume_delivered1); die;
                     ...
                     if( strtotime($l->goods_in_date) <= strtotime($month_last)) {        
...
//                    echo $goods_in_date = $j->goods_in_date;
...
//                 }  
                  ...
                  if(strtotime($delivery->delivery_date) > strtotime($start_date)||$delivery_lot->delivery_type!="Full"&&trim(array_sum($volume_delivered))!=trim($l->volume)){
                     if($delivery_lot->delivery_type=="Full"||trim(array_sum($volume_delivered))==trim($l->volume)){
                      $till_date = $delivery->delivery_date;
                      if(strtotime($delivery->delivery_date) > strtotime($month_last)){
                           $till_date_next[] = $month_last;
                           //$till_date = $month_last;
...
//                      \App\Libraries\ZnUtilities::pa($till_date_next);
//                      \App\Libraries\ZnUtilities::pa($volume_delivered);
//                      \App\Libraries\ZnUtilities::pa(count($till_date_next));
//                      \App\Libraries\ZnUtilities::pa($delivery->delivery_date);
//                      \App\Libraries\ZnUtilities::pa($j->goodsin_id);
                      
                   if(isset($till_date_next)?count($till_date_next):'' > 0){
                        $till_date = $month_last;
                   }
                     ...
                     $goods_in_date_goodsin = $j->goods_in_date;
                     if (strtotime($goods_in_date_goodsin) >= strtotime($start_date)&&strtotime($goods_in_date_goodsin) <= strtotime($month_last)) {
...
//            
                  ...
                  @endforeach
                       ...
                       <?php    //  $delivery = App\Models\Delivery::where('goodsin_id', $j->goodsin_id)->orderBy('id', 'DESC')->first();
...
//            $deliver_lots = \App\Models\DeliveryLot::where('goodsin_id',$j->goodsin_id)->where('delivery_type','Full')->lists('lot_id')->toArray();
//               $count=array();
//          foreach($lot as $k=>$d){
//         $delivery_prev_lot = App\Models\DeliveryLot::where('goodsin_id', $j->goodsin_id)->where('lot_id',$d->lot_id)->orderBy('delivery_id', 'DESC')->first();
//           
//         if((isset($delivery_prev_lot)?$delivery_prev_lot->delivery_type:'')!="Full"){
//          $delivery_lot = \App\Models\DeliveryLot::where('goodsin_id',$d->goodsin_id)->where('lot_id',$d->lot_id)->lists('volume_to_deliver')->toArray();
//          
//          $delivery = \App\Models\DeliveryLot::where('goodsin_id',$d->goodsin_id)->where('lot_id',$d->lot_id)->orderBy('lot_id','DESC')->first();
...
//                 if(array_sum($charge_rate)>0) { 
                  ...
                  <tr>
                        <td colspan="1" style="text-align:left;border-top: 1px solid #000; border-right:1px solid #000;border-left:1px solid #000; font-size: 14px;">{{in_array($j->goodsin_id,$array)? $j->goodsin_id :''}} </td>
                        <td colspan="1" style="text-align:left;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{in_array($j->goodsin_id,$array)? $j->project_name :''}} </td>
                        <td colspan="1" style="text-align:left;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{in_array($j->goodsin_id,$array)? $j->client_order_number :''}}</td>

                        <td colspan="2" style="text-align:left;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{in_array($j->goodsin_id,$array)? \App\Libraries\ZnUtilities::format_date($goods_in_date,10) :''}} </td>
                        <td colspan="2" style="text-align:left;border-top: 1px solid #000;border-right:1px solid #000; font-size: 14px;">{{in_array($j->goodsin_id,$array)? \App\Libraries\ZnUtilities::format_date($till_date,10) :''}} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;">{{'&#163;'}}{{in_array($j->goodsin_id,$array) ? number_format(array_sum($charge_rate), 2, '.', '') : '' }} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{(in_array($j->goodsin_id,$array)) ? $handling_charge:''}} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{in_array($j->goodsin_id,$array)?  number_format(array_sum($charge_rate) + $handling_charge , 2, '.', '') :''}}   </td>

    <?php  $total[] = array_sum($charge_rate) + ($handling_charge) //+ $loading_charge ;   ?>
              ...
              <?php //  }  ?>
                ...
                $delivery = App\Models\Delivery::where('goodsin_id', $j->goodsin_id)->orderBy('id', 'DESC')->first();
...
//                            \App\Libraries\ZnUtilities::pa('total  '.$days*$j->total_volume*$j->charge_rate);
                    ...
                    <tr>
                        ...
                        <td colspan="2" style="text-align:left;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{in_array($j->goodsin_id,$array)? \App\Libraries\ZnUtilities::format_date($goods_in_date,10) :''}} </td>
                        <td colspan="2" style="text-align:left;border-top: 1px solid #000;border-right:1px solid #000; font-size: 14px;">{{in_array($j->goodsin_id,$array)? \App\Libraries\ZnUtilities::format_date($till_date,10) :''}} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;">{{'&#163;'}}{{in_array($j->goodsin_id,$array) ? $charge_rate:'' }} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{in_array($j->goodsin_id,$array)? $handling_charge   :''}} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{in_array($j->goodsin_id,$array)?  number_format($charge_rate + $handling_charge , 2, '.', '') :''}}   </td>
