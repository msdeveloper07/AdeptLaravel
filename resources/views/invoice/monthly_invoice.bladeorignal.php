...
//                            \App\Libraries\ZnUtilities::pa('total  '.$count*$l->volume*$j->charge_rate);
                           if($till_date){  
                    $delivery_old = App\Models\Delivery::where('job_id', $j->id)->where('delivery_date','<=',$till_date)->orderBy('job_delivery_id','ASC')->get();    
...
//                 if(array_sum($charge_rate)>0) { 
                  ...
                  <tr>
                        ...
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{in_array($j->id,$array)?  number_format(array_sum($charge_rate) + $handling_charge , 2, '.', '') :''}}   </td>
