...
//                            \App\Libraries\ZnUtilities::pa('total  '.$count*$l->volume*$j->charge_rate);
                           if($till_date){
                    $delivery_old = App\Models\Delivery::where('goodsin_id', $j->goodsin_id)->where('delivery_date','<',$start_date)->orderBy('job_delivery_id','ASC')->get();    
...
//             
//         }
                 if( strtotime($l->goods_in_date) <= strtotime($month_last) && array_sum($charge_rate) > 0) { 
                    ?>
                  ...
                  <tr>
                        ...
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{(in_array($j->goodsin_id,$array) && $l->lot_id==1) ? $handling_charge   :'0.00'}} </td>
                        <td colspan="1" style="text-align:right;border-top: 1px solid #000; border-right:1px solid #000; font-size: 14px;"> {{'&#163;'}}{{in_array($j->goodsin_id,$array)?  number_format(array_sum($charge_rate) + ($l->lot_id==1 ?$handling_charge:'0.00') , 2, '.', '') :''}}   </td>
