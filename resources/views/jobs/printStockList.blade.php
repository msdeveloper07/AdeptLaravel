...
<!--            <tr>
         ...
         <tr>
            ...
            <tr>
                <td colspan="5" ><strong>PROJECT NAME:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{$job->project_name}}</strong></td>
            </tr>

            <tr><td colspan='5' >&nbsp;</td></tr>
            <tr><td colspan='5' >&nbsp;</td></tr>
             <tr>
                 <td colspan="5" style="text-align:right;"><strong>JOB NUMBER:&nbsp;{{$job->id}}</strong></td>
             </tr>
             <tr><td colspan='5' >&nbsp;</td></tr>
            
            <tr><td colspan='5' >&nbsp;</td></tr>
            <tr>
                <td colspan="5" style='text-align:right;'><strong>GOODS IN DATE:&nbsp;{{\App\Libraries\ZnUtilities::format_date($job->goods_in_date,10)}}</strong></td>
            </tr>
            <tr><td colspan='5' >&nbsp;</td></tr>
            <tr>
         ...
         @if(!in_array($l->lot_id,$deliver_lots))
          <?php
          $delivery_lot = App\Models\DeliveryLot::where('job_id',$l->job_id)->where('lot_id',$l->lot_id)->lists('volume_to_deliver')->toArray();
          
          $delivery = App\Models\DeliveryLot::where('job_id',$l->job_id)->where('lot_id',$l->lot_id)->orderBy('lot_id','DESC')->first();
...
//
  ...
  // \App\Libraries\ZnUtilities::pa($deliverd_lot);
          ...
          <tr>
                ...
                <td colspan='1' style="border-right: 1px solid #000;"><?php if($l->job_type=='Weekly/Dimension'){echo $total_vol - array_sum($delivery_lot); }?></td>
             </tr>
          <?php }} }else{
